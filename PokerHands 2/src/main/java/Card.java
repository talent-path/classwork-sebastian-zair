...
// method cointains an empty list
    // we iterate through suit and value (nested)
    // after each iteration we add the cards to the empty arrayList
    // List<String> name = new ArrayList<>();

    public static List<Card>  generateDeck() {
        // gerneate all possible 52 cards
        List<Card> deck = new ArrayList<>();

        for(FaceValue value: values) {
            for(Suit suit: suits) {
